...
class AuthenticatedSupabaseClient(
     ...
     */
    suspend fun getGroups(): List<com.urbanmesh.services.CheckboxGroupEntity> {
        return client.from("groups")
            .select()
            .decodeList<com.urbanmesh.services.CheckboxGroupEntity>()
    }
     ...
     * Uses the Supabase Postgrest client which properly handles RLS policies
     */
    suspend fun getGroupById(groupId: String): com.urbanmesh.services.CheckboxGroupEntity? {
        return client.from("groups")
            .select {
                filter {
                    eq("id", groupId)
            ...
            .decodeSingleOrNull<com.urbanmesh.services.CheckboxGroupEntity>()
    }
    ...
    suspend fun createGroup(
        ...
        description: String?,
        ownerId: String
    ): com.urbanmesh.services.CheckboxGroupEntity {
        val input = com.urbanmesh.services.CreateGroupInput(
            name = name,
        ...
        val response: HttpResponse = httpClient.post("$supabaseUrl/rest/v1/groups") {
            ...
            contentType(ContentType.Application.Json)
            setBody(json.encodeToString(com.urbanmesh.services.CreateGroupInput.serializer(), input))
        }
        val jsonString = response.bodyAsText()
        val groups = json.decodeFromString<List<com.urbanmesh.services.CheckboxGroupEntity>>(jsonString)
        return groups.first()
    }
     ...
     */
    suspend fun getGroupMembers(groupId: String): List<com.urbanmesh.services.GroupMemberEntity> {
    ...
    suspend fun addGroupMember(groupId: String, userId: String): com.urbanmesh.services.GroupMemberEntity {
        val input = com.urbanmesh.services.AddMemberInput(
            group_id = groupId,
            user_id = userId
        )
        val response: HttpResponse = httpClient.post("$supabaseUrl/rest/v1/group_members") {
            ...
            header("Prefer", "return=representation")
            contentType(ContentType.Application.Json)
            setBody(json.encodeToString(com.urbanmesh.services.AddMemberInput.serializer(), input))
        }
        val jsonString = response.bodyAsText()
        val members = json.decodeFromString<List<com.urbanmesh.services.GroupMemberEntity>>(jsonString)
